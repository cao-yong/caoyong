...
public class CartController {
    ...
    public String addCart(AddCartDTO addCartDTO, Model model, HttpServletRequest request,
                          HttpServletResponse response) {
        log.info("addCart start. addCartDTO:{}",
                ToStringBuilder.reflectionToString(addCartDTO, ToStringStyle.DEFAULT_STYLE));
        try {
            //获取购物车
    ...
    public String toCart(Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(username)) {
                ...
                //从redis中查询所有的购物车
                ResultBase<BuyerCart> BuyerCartResult = skuService.selectBuyerCartFromRedis(username);
                //redis中的
            ...
            if (!items.isEmpty()) {
                for (BuyerItem buyerItem : items) {
                    //查询sku详细信息
                    buyerItem.setSku(skuService.selectSkuById(buyerItem.getSku().getId()).getValue());
                }
            }
    ...
    public String trueBuy(Long[] skuIds, Model model, HttpServletRequest request, HttpServletResponse response) {
        log.info("trueBuy start, skuIds:{}", ToStringBuilder.reflectionToString(skuIds, ToStringStyle.DEFAULT_STYLE));
        try {
            String username = sessionProvider.getAttributeForUser(RequestUtil.getCSESSIONID(request, response));
            //从redis中查询所有的购物车
            ResultBase<BuyerCart> BuyerCartResult = skuService.selectBuyerCartFromRedis(username);
            ...
            log.error("getBuyerCartFromCookie biz error:{}", e.getMessage(), e);
        } catch (Exception e) {
                ...
                for (Cookie cookie : cookies) {
                    if (Constants.BUYER_CART.equals(cookie.getName())) {
                        ...
                        buyerCart = StringUtils.isNotBlank(value)
                                ? JSONConversionUtil.stringToObj(value, BuyerCart.class) : null;
                        break;
                    }
                }
                //java8实现
                /*
                 * String buyerCartCookie = Stream.of(cookies) .filter(cookie ->
                 * Constants.BUYER_CART.equals(cookie.getName())) .findFirst()
                 * .map(Cookie :: getValue).get(); buyerCart =
                 * JSONConversionUtil.stringToObj(buyerCartCookie,
                 * BuyerCart.class);
                 */
            }
            //当购物车为空时创建购物车
     ...
     * @param response
     */
    private void saveBuyerCartToRedis(BuyerCart buyerCart, String username, HttpServletResponse response) {
