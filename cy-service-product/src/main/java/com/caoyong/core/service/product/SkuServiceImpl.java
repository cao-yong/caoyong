...
public class SkuServiceImpl implements SkuService {
	...
	public ResultBase<Integer> insertBuyerCartToRedis(BuyerCart buyerCart, String username) throws BizException{
		log.info("insertBuyerCartToRedis start. buyerCart:{}", ToStringBuilder.
				reflectionToString(buyerCart, ToStringStyle.DEFAULT_STYLE));
		...
		try {
			...
			if(null != buyerCart && !items.isEmpty()){
				items.stream().forEach(item -> {
					...
					//累计
					Boolean hexists = jedis.hexists(key, 
