...
public class LoginController {
    ...
    public String login(LoginDTO loginDTO, Model model, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            ResultBase<Buyer> buyerResult = buyerService.selectBuyerByUsername(loginDTO.getUsername());
            ...
            //保存用户名到session中
            sessionProvider.setAttributeForUser(RequestUtil.getCSESSIONID(request, response), loginDTO.getUsername());

            //返回之前访问的页面
            return "redirect:"
                    + (StringUtils.isNotBlank(loginDTO.getReturnUrl()) ? loginDTO.getReturnUrl() : "localhost:8082/");
