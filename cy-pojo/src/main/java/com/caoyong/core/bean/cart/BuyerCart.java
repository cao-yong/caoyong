...
public class BuyerCart implements Serializable{
	...
	public void addItem(BuyerItem item){
		...
		if(items.contains(item)){
			//累计
			items.stream().filter(it -> it.equals(item))
						  .forEach(it -> it.setAmount
						  (it.getAmount() + item.getAmount()));
		}else{
			items.add(item);
		}
	}
	...
	public String getProductPrice(){
		...
		if(!items.isEmpty()){
			...
			for(BuyerItem item : items){
				price = price.add(new BigDecimal(item.getSku().getPrice())
	...
	public String getFee(){
		String result  = (new BigDecimal(getProductPrice()).compareTo
				(new BigDecimal("79.00"))) == -1 ? 
	...
	public String getTotalPrice(){
		String result = new BigDecimal(getProductPrice())
				.add(new BigDecimal(getFee())).toString();
		return result;
	}
}
